...
bool j1App::LoadConfig()
{
	...
	pugi::xml_parse_result res = savegame_file.load_file("savegame.xml");

	if (res == NULL)
	{
		LOG("Could not load map xml file savegame.xml. pugi error: %s", res.description());
		ret = false;
	}
	else
	{
...
bool j1App::CleanUp()
{
	...
	savegame.child("camera_x").first_child().set_value(x);
	savegame.child("camera_y").first_child().set_value(x);
...
void j1App::RealLoad() {
	...
	// self-config
	title.create(savegame_file.child("title").child_value());
	organization.create(savegame_file.child("organization").child_value());
...
void j1App::RealSave() const {
	...
	App->savegame_file.save_file("savegame.xml");
}
